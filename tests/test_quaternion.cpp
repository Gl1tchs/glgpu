...
TEST_CASE("Quaternion Rotations", "[Quat]") {
	SECTION("Axis-Angle rotation of 90 degrees around Z axis") {
		...
		gpukit::Quat q = gpukit::Quat::angle_axis(M_PI / 2.0f, gpukit::Vec3f{ 0.0f, 0.0f, 1.0f });
		gpukit::Vec3f v(1.0f, 0.0f, 0.0f);
		gpukit::Vec3f rotated = q * v;
	...
	SECTION("Euler Angles pitch/yaw/roll") {
		...
		gpukit::Quat q = gpukit::Quat::euler(0.0f, M_PI / 2.0f, 0.0f);
		gpukit::Vec3f v(1.0f, 0.0f, 0.0f);
		gpukit::Vec3f rotated = q * v;
	...
	SECTION("Conversion to Matrix and Back") {
		gpukit::Quat q1 = gpukit::Quat::angle_axis(0.5f, gpukit::Vec3f{ 1.0f, 2.0f, 3.0f }).normalized();
		gpukit::Mat3 m = q1.to_mat3();
		gpukit::Quat q2 = gpukit::Quat::from_mat3(m);
		...
		bool match = (q1 == q2) || (q1 == gpukit::Quat(-q2.x, -q2.y, -q2.z, -q2.w));
		REQUIRE(match);
	}

	SECTION("Slerp Interpolation") {
		gpukit::Quat q0 = gpukit::Quat::angle_axis(0.0f, gpukit::Vec3f{ 0.0f, 0.0f, 1.0f });
		gpukit::Quat q1 = gpukit::Quat::angle_axis(M_PI / 2.0f, gpukit::Vec3f{ 0.0f, 0.0f, 1.0f });
		gpukit::Quat q_half = gpukit::Quat::slerp(q0, q1, 0.5f);

		gpukit::Quat expected = gpukit::Quat::angle_axis(M_PI / 4.0f, gpukit::Vec3f{ 0.0f, 0.0f, 1.0f });
