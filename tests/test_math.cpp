...
TEST_CASE("Math Utilities", "[Math]") {
	SECTION("Degrees to Radians") {
		float deg = 180.0f;
		float rad = math::as_radians(deg);
		REQUIRE_THAT(rad, Catch::Matchers::WithinRel(3.14159265f, 0.0001f));

		REQUIRE(math::as_radians(0.0f) == 0.0f);

		float deg90 = 90.0f;
		REQUIRE_THAT(math::as_radians(deg90), Catch::Matchers::WithinRel(1.570796f, 0.0001f));
	}
}
...
TEST_CASE("Vector Min Functions", "[Math][Min]") {
	SECTION("Vec2 Min") {
		...
		REQUIRE(math::min(a, b) == Vec2f(1.0f, 5.0f));

		// Scalar: min(1, 4)=1, min(10, 4)=4
		REQUIRE(math::min(a, 4.0f) == Vec2f(1.0f, 4.0f));
	}
	...
	SECTION("Vec3 Min") {
		...
		REQUIRE(math::min(a, b) == Vec3f(5.0f, 20.0f, 15.0f));
		REQUIRE(math::min(a, 15.0f) == Vec3f(10.0f, 15.0f, 15.0f));
	}
	...
	SECTION("Vec4 Min") {
		...
		REQUIRE(math::min(a, b) == Vec4f(1.0f, 2.0f, 2.0f, 1.0f));
		REQUIRE(math::min(a, 2.5f) == Vec4f(1.0f, 2.0f, 2.5f, 2.5f));
	}
}
...
TEST_CASE("Vector Max Functions", "[Math][Max]") {
	SECTION("Vec2 Max") {
		...
		REQUIRE(math::max(a, b) == Vec2f(5.0f, 10.0f));

		// Scalar: max(1, 4)=4, max(10, 4)=10
		REQUIRE(math::max(a, 4.0f) == Vec2f(4.0f, 10.0f));
	}
	...
	SECTION("Vec3 Max") {
		...
		REQUIRE(math::max(a, b) == Vec3f(10.0f, 25.0f, 30.0f));
		REQUIRE(math::max(a, 15.0f) == Vec3f(15.0f, 20.0f, 30.0f));
	}
	...
	SECTION("Vec4 Max") {
		...
		REQUIRE(math::max(a, b) == Vec4f(4.0f, 3.0f, 3.0f, 4.0f));
		REQUIRE(math::max(a, 2.5f) == Vec4f(2.5f, 2.5f, 3.0f, 4.0f));
	}
}
...
TEST_CASE("Clamping Example", "[Math][Integration]") {
	...
	Vec3f clamped = math::clamp(v, lower, upper);
