...
TEST_CASE("Image Management", "[image]") {
	...
	SECTION("Create Invalid Image (Zero Size)") {
		...
		if (img_res.is_error()) {
			...
			backend->image_free(img_res.value());
		}
	}
}
