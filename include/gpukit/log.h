...
template <typename... Args> void trace(GPUKIT_FMT_STRING(Args...) fmt, Args&&... args) {
	_write(LogLevel::Trace, GPUKIT_FMT_FORMAT(fmt, std::forward<Args>(args)...));
}

template <typename... Args> void info(GPUKIT_FMT_STRING(Args...) fmt, Args&&... args) {
	_write(LogLevel::Info, GPUKIT_FMT_FORMAT(fmt, std::forward<Args>(args)...));
}

template <typename... Args> void warn(GPUKIT_FMT_STRING(Args...) fmt, Args&&... args) {
	_write(LogLevel::Warning, GPUKIT_FMT_FORMAT(fmt, std::forward<Args>(args)...));
}

template <typename... Args> void error(GPUKIT_FMT_STRING(Args...) fmt, Args&&... args) {
	_write(LogLevel::Error, GPUKIT_FMT_FORMAT(fmt, std::forward<Args>(args)...));
}

template <typename... Args> void fatal(GPUKIT_FMT_STRING(Args...) fmt, Args&&... args) {
